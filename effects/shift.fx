...
texture Control <string uiname="Control";>;
sampler CSamp = sampler_state    //sampler for doing the texture-lookup
{
    Texture   = (Control);          //apply a texture to the sampler
    MipFilter = LINEAR;         //sampler states
    MinFilter = LINEAR;
    MagFilter = LINEAR;
};
...
{
    float4 ct = tex2D(CSamp, In.TexCd);
	float2 uv = In.TexCd + (ct.rg-float2(0.5, 0.5)) * power;
