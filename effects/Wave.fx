...
//texture
texture NTex <string uiname="NTexture";>;
    ...
    float4 TexCd : TEXCOORD0)
{
	...
	float z = tex2Dlod(Samp, float4(PosO.xy+0.5, 0, 0));
	float4 p = PosO + float4(0, 0, z*0.5, 0);
    Out.Pos = mul(p, tWVP);
	
    //transform texturecoordinates
    Out.TexCd = mul(TexCd, tTex);
	float4 n = normalize(tex2Dlod(NSamp, float4(PosO.xy+0.5, 0, 0)));
	Out.Col = dot(n, normalize(float4(10, 10, 1, 1)));
	Out.Col.w = 1;
